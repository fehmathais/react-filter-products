...
export class SearchPageObject {
  ...
  async fillPriceFromInput(value) {
    act(() => {
      fireEvent.change(this.elements.priceFrom, {
        target: { value: value, name: "priceFrom" },
      });
    });
  ...
  async fillPriceToInput(value) {
    act(() => {
      fireEvent.change(this.elements.priceTo, {
        target: { value: value, name: "priceTo" },
      });
    });
    return await this.waitForComponentToUpdate();
  }
  ...
  displayedRowsCount() {
    return this.handler.container.querySelectorAll("#product-list tbody tr")
      .length;
  }
  ...
  getProductListHeader() {
    return this.handler.container.querySelector("#product-list header")
      .textContent;
  }

  getProductRowCells(rowIdx) {
    const cells = this.handler.container
      .querySelectorAll("#product-list tbody tr")
      [rowIdx].querySelectorAll("td");
    console.log(cells);
    return [...cells].map((node) => node.textContent);
  }

  getToggleColumnsLabels() {
    const labels = this.handler.container.querySelectorAll(
